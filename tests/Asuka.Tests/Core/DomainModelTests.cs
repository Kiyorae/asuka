...
public sealed class DomainModelTests
{
    ...
    public void GeneratedProtocolIdentifiersAreNumericAndFitInt64()
    {
        var ids = Enumerable.Range(0, 100)
            .SelectMany(_ => new[] { Asuka.Core.IdGenerator.UserId(), Asuka.Core.IdGenerator.GroupId(), Asuka.Core.IdGenerator.MessageId() })
            .ToArray();
        ...
        var first = Asuka.Core.IdGenerator.EncodeTimestampForTesting(1_800_000_000_000L);
        var afterFormerWrapPeriod = Asuka.Core.IdGenerator.EncodeTimestampForTesting(1_800_100_000_000L);
    ...
    {
        var integer = Asuka.Core.JsonValue.Parse("1");
        var floatingPoint = Asuka.Core.JsonValue.Parse("1.0");

        Assert.AreEqual(integer, floatingPoint);
        Assert.AreEqual(integer.GetHashCode(), floatingPoint.GetHashCode());

        Asuka.Core.JsonValue unset = default;
        Assert.AreEqual(Asuka.Core.JsonValue.Null, unset);
    ...
    {
        var directory = Path.Combine(Path.GetTempPath(), $"asuka-assets-{Guid.NewGuid():N}");
        var sourcePath = Path.Combine(directory, "source.txt");
        Directory.CreateDirectory(directory);
        await File.WriteAllTextAsync(sourcePath, "asuka", Encoding.UTF8);
        try
        {
            using var store = new Asuka.Core.AssetStore(Path.Combine(directory, "store"));
            ...
            Assert.AreEqual(Asuka.Core.AssetSourceKind.Local, fromPath.Source.Kind);
    ...
    public async Task AssetStoreDoesNotFollowRedirects()
    {
        ...
        try
        {
            using var store = new Asuka.Core.AssetStore(directory, handler);
