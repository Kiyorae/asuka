...
internal sealed class ProtocolTestFixture : IAsyncDisposable
{
    ...
    private readonly string _directory = Path.Combine(
        Path.GetTempPath(),
        $"matcha-protocol-tests-{Guid.NewGuid():N}");

    internal ProtocolTestFixture()
    {
        Store = new MatchaStore();
    ...
    internal MatchaStore Store { get; }
    ...
    {
        await Store.SaveAsync(new User("Matcha", id: SelfId, nickname: "Matcha Bot"));
