...
public sealed partial class LogConsoleWindow : Window
{
    ...
    public LogConsoleWindow(AppEnvironment environment)
    {
        ...
        Title = "Asuka Log Console";
        WindowChrome.Configure(this, 1060, 700);
        ...
        Closed += LogConsoleWindow_Closed;
    ...
    private void LogConsoleWindow_Closed(object sender, WindowEventArgs args)
    {
        _refreshTimer.Stop();
    ...
    private void CategoryBox_SelectionChanged(object sender, SelectionChangedEventArgs e) => RefreshFilter();

    private void RefreshFilter()
    {
        if (SearchBox is null || CategoryBox is null)
        {
            return;
        }

        var allEntries = GetActivityEntries();
        ...
        {
            LogList.SelectedItems.Add(activeEntry);
        }
    ...
    private async void Export_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            var picker = new FileSavePicker
            {
                ...
                SuggestedFileName = $"asuka-log-{DateTimeOffset.Now:yyyyMMdd-HHmmss}",
            };
            ...
            if (file is not null)
            {
                await Windows.Storage.FileIO.WriteTextAsync(file, FormatEntries(GetActivityEntries()));
    ...
    private List<LogEntryItem> GetActivityEntries() => _environment.Logs
